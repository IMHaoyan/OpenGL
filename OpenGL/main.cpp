...
// camera
Camera camera(vec3(-2.3f,0.25f,5.0f));
...
int main()
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);// 设置灯立方体的顶点属性（对我们的灯来说仅仅只有位置数据）
    glEnableVertexAttribArray(0);
    ...
    camera.Front = vec3(0.58f, 0.045f, -0.8f);
    // render loop
    // -----------
